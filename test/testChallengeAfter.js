...
contract("Plasma ERC721 - Exit Spent Coin Challenge / `challengeAfter`", async function(accounts) {
    ...
    const COINS = [new BN("1"), new BN("2"), new BN("3")];
    ...
    beforeEach(async function() {
        ...
        cards = await SampleNFTs.new(plasma.address);
        await cards.register({from: alice});
        assert.equal(await cards.balanceOf.call(alice), 5);

        let ret;
        for (let i = 0; i < ALICE_DEPOSITED_COINS; i ++) {
            // TODO(vamsi): there are two variants of the `safeTransferFrom` function
            // (see: https://github.com/ethereum/EIPs/blob/master/EIPS/eip-721.md), one that
            // takes in a fourth `data` argument and one that doesn't but simply proxies a call
            // to the former function. Investigate why we have to pass in the fourth argument
            // for this to work.
            await cards.safeTransferFrom(alice, plasma.address, COINS[i], '0x0', { from: alice })
        }
