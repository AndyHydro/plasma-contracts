...
contract('Plasma Cash - Basic Flow', async function(accounts) {
  ...
  const denominations = [3000 * DECIMALS, 2000 * DECIMALS, 4000 * DECIMALS];
  ...
  beforeEach(async function() {
    ...
    erc20 = await SampleToken.new({ from: authority });
    erc721 = await SampleNFTs.new(plasma.address, { from: authority });
    ...
    for (let i = 0; i < denominations.length; i++) {
      ...
      await erc20.approve(plasma.address, denominations[i], { from: alice })
      await plasma.depositERC20(denominations[i], erc20.address, { from: alice })

      await erc721.approve(plasma.address, coins[i], { from: alice })
      await plasma.depositERC721(coins[i], erc721.address, { from: alice })
    }
    ...
    assert.equal(
      await web3.eth.getBalance(plasma.address),
      web3.toWei(10, 'ether')
    );

    const depositEvent = plasma.Deposit(
      {},
  ...
  describe('Exit of an ERC20, an ERC721 and an ETH coin', function() {
    it('Directly after their deposit', async function() {
      let UTXO = [
        {
          slot: events[0]['args'].slot,
          block: events[0]['args'].blockNumber.toNumber()
        },
        {
          slot: events[1]['args'].slot,
          block: events[1]['args'].blockNumber.toNumber()
        },
        {
          slot: events[2]['args'].slot,
          block: events[2]['args'].blockNumber.toNumber()
        }
      ];
